...
//	*(m_pTableZero + pskNewKey->keyword[0]) = KEYWORD;
	*(m_pTableZero + key.keyword[0]) = KEYWORD;

	m_pskKeyword.insert( std::make_pair(Keyword, key) );
...
void CSyntaxColorizer::ClearKeywordList()
{
	...
	for(std::map<std::string, COLORIZE_KEYWORD>::iterator it = m_pskKeyword.begin(); it != m_pskKeyword.end(); ++it)
	{
		COLORIZE_KEYWORD& key = (*it).second;
		*(m_pTableZero + key.keyword[0]) = SKIP;

		if(_stricmp(key.keyword.c_str(),"rem") == 0)
			*(m_pTableTwo + '\n') = SKIP;
	}
	m_pskKeyword.clear();
...
void CSyntaxColorizer::Colorize(long nStartChar, long nEndChar, CRichEditCtrl *pCtrl)
{
	...
	CString strBuff = pCtrl->GetSelText();
	pCtrl->SetSelectionCharFormat(m_cfDefault);

	colorize(strBuff.GetBuffer( strBuff.GetLength() ), pCtrl, nStartChar);
...
void CSyntaxColorizer::colorize(LPTSTR lpszBuf, CRichEditCtrl *pCtrl, long iOffset /*=0*/)
{
	...
	while(lpszBuf[x])
	{
		...
		case KEYWORD:
			...
			{
				COLORIZE_KEYWORD& key = (*it).second;
				std::string word = key.keyword;

				if( word[0] == lpszTemp[0] )
				{
					int x1 = 0, y1 = 0; 
					iStart = iOffset + x;

					std::string word2(lpszTemp, word.length() );
					
					/*
					while( word[x1] )
					{
						y1 += lpszTemp[x1] ^ word[x1];
						x1++;
					}
					*/
					...
					if(/*y1 == 0*/ 
							word ==	word2 && 
							(*(m_pAllowable + (lpszBuf[x-1])) &&  
							 *(m_pAllowable + (lpszBuf[ x + word.length()] ))))
					{
						if(_stricmp(word.c_str(),"rem") == 0)
						{
							pTable = m_pTableThree;
						}
						else 
						{
							...
							pCtrl->SetSelectionCharFormat(key.cf);
						}
					}
				}
			}
