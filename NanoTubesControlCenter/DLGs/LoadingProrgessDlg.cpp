...
BOOL CLoadingProrgessDlg::OnInitDialog()
{
	...
	CString strInfo = CNanoTubesControlCenterApp::GetAppInfo();

	GetDlgItem(IDC_TEXT)->SetWindowText(strInfo);
	//
	// Show size gripper
	//
...
// 		);
	...
	{
		{ _T("Section"),     fullWidth/2, fullWidth/2, fullWidth/2,   0.0, __EGCS_TA_HORZ_LEFT },
		{ _T("Progress"),   fullWidth/2, fullWidth/2, fullWidth/2,   0.0, __EGCS_TA_HORZ_CENTER },
	};
...
long CLoadingProrgessDlg::AddLoadingSection(DOCUMENT_SECTION_INFO& sec)
{
	...
	pCellProgress->TextSet( _T("Processed : ") );
